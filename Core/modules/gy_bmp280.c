...
void bmp280_setup_Standard(BMP280 *new_setup)
{
	new_setup->setup.mode = NORMAL_MODE;
	new_setup->setup.filter = FILTER_OFF;
	new_setup->setup.oversampling_temp = SR;
	new_setup->setup.oversampling_press = SR;
	new_setup->setup.standby = STANDBY_250;
}

void bmp280_setup_Drone(BMP280 *new_setup)
{
	new_setup->setup.mode = NORMAL_MODE;
	new_setup->setup.filter = FILTER_16;
	new_setup->setup.oversampling_temp = LP;
	new_setup->setup.oversampling_press = UHR;
	new_setup->setup.standby = STANDBY_0_5;
}

void bmp280_setup_Elevator(BMP280 *new_setup)
{
	new_setup->setup.mode = NORMAL_MODE;
	new_setup->setup.filter = FILTER_4;
	new_setup->setup.oversampling_temp = ULP;
	new_setup->setup.oversampling_press = SR;
	new_setup->setup.standby = STANDBY_125;
}

void bmp280_setup_DropDetect(BMP280 *new_setup)
{
	new_setup->setup.mode = NORMAL_MODE;
	new_setup->setup.filter = FILTER_OFF;
	new_setup->setup.oversampling_temp = ULP;
	new_setup->setup.oversampling_press = LP;
	new_setup->setup.standby = STANDBY_0_5;
}
...
{
	uint8_t rx_buff;
	if (HAL_I2C_Mem_Read(BMP280_I2C, BMP280_ADDRESS, addr, 1, &rx_buff, 1, i2c_timeout)
			== HAL_OK)
	{
		*value = rx_buff;
		return 1;
...
{
	uint8_t rx_buff[2];
	if (HAL_I2C_Mem_Read(BMP280_I2C, BMP280_ADDRESS, addr, 1, &rx_buff, 2, i2c_timeout)
			== HAL_OK)
	{
...
{
	uint8_t rx = value;
	if (HAL_I2C_Mem_Write(BMP280_I2C, BMP280_ADDRESS, addr, 1, &rx, 1, i2c_timeout)
			== HAL_OK)
		return 1;
	else
		return 0;
...
{
	if (HAL_I2C_Mem_Read(BMP280_I2C, BMP280_ADDRESS, addr, 1, value, len, i2c_timeout)
			== HAL_OK)
		return 1;
	else
		return 0;
...
bool bmp280_init(BMP280 * bmp)
{
	...
	if (bmp->setup.mode == FORCE_MODE) bmp->setup.mode = SLEEP_MODE;
	uint8_t ctrl = 	(bmp->setup.oversampling_temp << 5) | 
				   	(bmp->setup.oversampling_press << 2)|
					bmp->setup.mode;
	if (!write_8bit_register(&ctrl, CTRL_MEAS)) return false;
	uint8_t config = (bmp->setup.standby << 5) | (bmp->setup.filter << 2);
	if (!write_8bit_register(&config, CONFIG)) return false;
	// init ref altitude 
	bmp->ref_altitude = 0;
	// get ref altitude 
	double raw_altitude = 0; 
	// get t_fine
	bmp280_get_temp(bmp);

	for (int i = 0; i < TIME_REF; i++)
	{
		raw_altitude += bmp280_get_altitude(bmp);
		HAL_Delay(1);
	}
	bmp->ref_altitude = raw_altitude / TIME_REF;
	return true;
}
...
double bmp280_get_altitude (BMP280 * bmp)
{
	...
	double altitude = 44330 * (1.0 - pow(p / (1013.25 * 100), 0.1903)) - bmp->ref_altitude;
	return altitude;
}
